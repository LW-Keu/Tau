...
LOG_FILE="scripts/run/tau_api.log"
...
terminate_tau_api_pid() {
    local pid="$1"
    info "正在终止进程 PID=${pid} ..."
    if ! is_tau_api_pid "$pid"; then
...
if [ -n "$CONFLICT_INFO" ]; then
    ...
    PIDS="$VERIFIED_PIDS"
    if [ -z "${PIDS// }" ]; then
        error "未找到可验证的 tau api 进程，拒绝发送信号。"
        exit 1
    fi
    echo "  ────────────────────────────────────────────────────────────"
    echo ""
    ...
    # ---------- Step 2: 询问用户 ----------
    printf "是否需要 kill 掉占用端口的进程并启动 tau api？[yes/no] "
    read -r USER_REPLY

    if [ "$USER_REPLY" = "yes" ] || [ "$USER_REPLY" = "y" ]; then
        echo ""
        for pid in $PIDS; do
            terminate_tau_api_pid "$pid" || exit 1
        done
...
nohup tau api --port "$PORT" > "$LOG_FILE" 2>&1 &
TAU_API_PID=$!
echo "$TAU_API_PID" > "$PID_FILE"
...
echo ""
print_separator
print_kv "tau api 启动详情" ""
print_separator
if kill -0 "$TAU_API_PID" 2>/dev/null; then
    print_kv "状态" "✅ 运行中"
else
    print_kv "状态" "❌ 已退出"
fi
print_kv "PID" "$TAU_API_PID"
...
echo ""
print_kv "进程详情 (ps)" ""
echo "  $(ps -p "$TAU_API_PID" -o pid,ppid,user,%cpu,%mem,etime,args --no-headers 2>/dev/null || echo '(进程已退出)')"
echo ""
...
# ---------- Step 6: 最终状态 ----------
if kill -0 "$TAU_API_PID" 2>/dev/null; then
    info "tau api 启动成功！PID=${TAU_API_PID}"
    info "查看实时日志: tail -f ${WORK_DIR}/${LOG_FILE}"
    info "停止服务:     kill ${TAU_API_PID}"
else
    error "tau api 进程已退出，请检查日志: ${WORK_DIR}/${LOG_FILE}"
    exit 1
fi
