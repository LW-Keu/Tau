...
> 用 Codex / AI 改 Tau 源码前读这一页。只列 **Tau 专属**约定;通用代码标准见下方权威来源,不在此重复。
...
- **结构勿大重排**:运行时包在 `src/tau_coding`、`src/tau_agent`、`src/tau_ai`;`TMWebDriver` 在 `external/`(外部组件);前端一律在 `apps/`(`common/tui/web/im/gui/pet/desktop/hub`)。不要为「更整洁」搬动目录。
- **顶层目录职责**(术语以 [`CONTEXT.md`](CONTEXT.md) 为准):`assets/`=运行时只读资产、`setup/`=设置向导、`examples/`=示例、`external/`=外部组件、`scripts/`=开发期烟测、`memory/`=agent 工具+SOP、`tests/`=测试。新文件先对号入座,对不上就先问。
