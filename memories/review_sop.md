...
任意前端 (TUI / Streamlit / wechat / desktop)
   └─ apps/common/review_cmd.py     ← 命令分发,剥 "/review" 前缀,注入 user_request
       └─ memories/review_sop/review_inline_prompt.txt   ← 完整 in-session 协议
           └─ memories/code_review_principles.md         ← 15 条好代码原则
```

- `review_cmd.py:install()` —— monkey-patch `Tau._handle_slash_cmd`,统一接管 `/review`
...
`file_read("memories/code_review_principles.md")` —— 15 条好代码原则,**每条 finding 必须
能映射到其中一条**。
...
> 每条规则的展开详见 `memories/review_sop/review_inline_prompt.txt` §5。
...
> 展开详见 `memories/review_sop/review_inline_prompt.txt` §6。
...
- **自定义评审条目**:编辑 `memories/code_review_principles.md`,reviewer 启动时整段注入
- **触发更换**:要把 `/review` 改成别的命令,只动 `apps/common/review_cmd.py` 的 `install()` 一处
