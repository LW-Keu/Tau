...
> **v3 变更**：邮件模块从 `mailer/` 迁至 `memories/`（库）+ `setup/configure_tauchain.py`（v2.2+ 活跃维护的人类交互式向导）。授权码从 keychain 改为 `.tau/tauchain.json` 明文存储（受 `.gitignore` + 文件权限 `600` 保护）。
...
- **2.1 发件邮箱地址**（如 lllilililll@qq.com）—— *唯一不可默认*
  校验：调 `from memories.email_config import infer_provider; infer_provider(addr)` 试一下。
...
调 `infer_provider(addr)`（`memories.email_config.infer_provider`）。
...
调 `memories.email_config.save_email_config(cfg)`：
...
- ✅ **v3 单一 API**：所有写操作走 `memories.email_config.save_email_config(cfg)`，禁止直接 `json.dump` 写 `.tau/tauchain.json`（绕开校验 + chmod）

## 阶段 5 · 验证

调 `python memories/email_report.py`（无参 send() 会自动从 `.tau/tauchain.json` 读 cfg + 密码，发测试邮件）。

或更通用：直接调 `memories.email_report.send()`（库 API）。
...
设计要点：
- **复用库**：写入与字段契约复用 `memories.email_config.save_email_config`（不再自带写逻辑），杜绝字段漂移；按邮箱域名调 `memories.email_config.infer_provider` 自动推断 SMTP。
- **交互 + 非交互双模式**：
  ...
  - 配置后验证：`--send-test` 调 `memories.email_report.send()` 发一封测试邮件
...
用户说"重试发测试邮件" → 跳回阶段 5（**不重做 2.1-2.4**，直接跑 `python memories/email_report.py`）。如果用户说"重配"，回到阶段 2.1。
