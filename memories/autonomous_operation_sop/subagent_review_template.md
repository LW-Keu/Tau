...
评审 temp/TODO.txt 的 [ ] TODO。读 memories/global_mem.txt + memories/autonomous_operation_sop/task_planning.md 了解环境。逐条 1-10 评分, 输出 temp/$TASK/review.md >=5KB, 7+ 节。
EOF
...
mkdir -p temp/$TASK
cat > temp/$TASK/input.txt <<EOF
评审 temp/TODO.txt 的 [ ] TODO。读 memories/global_mem.txt + memories/autonomous_operation_sop/task_planning.md。逐条 1-10 评分 + 理由, 输出 temp/$TASK/review.md >=5KB 7+ 节。参考范式: temp/review.md (2026-06-16, 245行, 8节, 6 TODO)。
EOF
python taumain.py --task $TASK --llm_no 0
# 轮询: while ! grep -q "review.md.*写入" temp/$TASK/output.txt; do sleep 180; done
# 验收: bash memories/autonomous_operation_sop/subagent_review_template_audit.sh $TASK
