...
1. 复制 `memories/vision_api.template.py` → `memories/vision_api.py`
2. 只改头部"用户配置区"：去 `.tau/taukey.py` 里扫描变量名（⚠️ 只看名字，禁止输出 apikey 值），尝试找能用配置名填入 `CLAUDE_CONFIG_KEY` / `OPENAI_CONFIG_KEY`，`DEFAULT_BACKEND` 选后端，并测试
