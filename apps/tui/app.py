...
FRONTENDS_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
...
_SETTINGS_PATH = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "..", "temp", "tui_settings.json"
)
...
class TauTUI(App[None]):
    ...
    def __init__(self, agent_factory: Optional[AgentFactory] = None) -> None:
        ...
        try:
            import apps.common.cost_tracker as cost_tracker; cost_tracker.install()
        except Exception:
            pass
        ...
        try:
            import apps.common.session_names as session_names; session_names.gc()
        except Exception:
            pass
    ...
    def on_mount(self) -> None:
        ...
        self._system("Welcome to Tau TUI. 按 / 唤起命令面板，Ctrl+N 新建会话。")
    ...
    def _cmd_rename(self, args, raw):
        ...
        # writes to (see taumain.Tau.__init__); exclude its basename
        ...
        try:
            import apps.common.session_names as session_names
    ...
    def _cmd_continue(self, args, raw):
        ...
        choices = []
        try:
            import apps.common.session_names as _sn
        except Exception:
            _sn = None
    ...
    def _do_continue_restore(self, path: str) -> str:
        sess = self.current
        from apps.common.continue_cmd import reset_conversation, restore
        try:
    ...
    def _cmd_cost(self, args, raw):
        try:
            import apps.common.cost_tracker as cost_tracker
        except Exception as e:
    ...
    def _update_plan_state(self, sess: AgentSession, _stream_text: str = "") -> None:
        import apps.common.plan_state as plan_state
...
def _warn_mintty():
    ...
    if direct_mintty:
        print(
            ...
            "    - Windows Terminal Git Bash: wt -p \"Git Bash\" python apps/tui/app.py\n"
            "    - Windows Terminal:          wt python apps/tui/app.py\n"
            "    - CMD:                       python apps\\tui\\app.py\n"
            "    - PowerShell:                python apps/tui/app.py\n"
            "\n"
