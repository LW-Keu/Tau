...
def find_default_tau_root() -> Path:
    candidates = [
        APP_DIR / "..",
        APP_DIR / ".." / "..",
        APP_DIR / ".." / "Tau",
...
DEFAULT_TAU_ROOT = find_default_tau_root()
...
class AgentManager:
    def __init__(self):
        ...
        self.tau_root = str(DEFAULT_TAU_ROOT)
    ...
    def taukey_path(self) -> str:
        return str(Path(self.tau_root) / ".tau" / "taukey.py")

    def ensure_ga_import_path(self) -> Path:
        root = Path(self.tau_root).resolve()
        return root
    ...
    def create_session(self, cwd: Optional[str] = None) -> Session:
        ...
        sess = Session(id=sid, cwd=str(cwd or self.tau_root))
        with self.lock:
    ...
    def run_agent_turn(self, sess: Session, prompt: str, images: Optional[list] = None):
        try:
            ...
            if hasattr(agent, "put_task"):
                ...
                full = "Tau object has no put_task/run method"
            if not full:
                full = "(completed)"
            if sess.cancel_requested:
                with self.lock:
...
async def get_config_handler(request):
    return json_ok({"tauRoot": manager.tau_root, "taukeyPath": manager.taukey_path, "config": manager.config})
...
async def save_config_handler(request):
    ...
    return json_ok({"ok": True, "tauRoot": manager.tau_root, "taukeyPath": manager.taukey_path, "config": manager.config})
...
async def path_open_handler(request):
    ...
    kind = data.get("kind", "")
    if kind == "taukey":
        target = Path(manager.tau_root) / ".tau" / "taukey.py"
    else:
        target = Path(data.get("path") or data.get("target") or manager.tau_root)
