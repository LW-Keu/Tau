...
apps_root = os.path.dirname(os.path.dirname(script_dir))  # apps/
project_root = os.path.dirname(apps_root)                  # Tau/
...
def init():
    ...
    if agent.llmclient is None:
        st.error("⚠️ Please set .tau/taukey.py — run `tau configure`!")
        st.stop()
...
def render_sidebar():
    ...
    if st.button(T('reinject_tools')):
        ...
        try:
            hist_path = os.path.join(project_root, 'assets', 'tool_usable_history.json')
    ...
    if st.button(T('desktop_pet')):
        ...
        pet_script = os.path.join(apps_root, 'pet', 'pet_v2.pyw')
        if not os.path.exists(pet_script): pet_script = os.path.join(apps_root, 'pet', 'pet_v1.pyw')
