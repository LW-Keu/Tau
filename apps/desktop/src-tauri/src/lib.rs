...
/// Get project root (parent of apps/)
fn project_root() -> PathBuf {
    std::env::current_exe()
        .expect("cannot get exe path")
        .parent().expect("cannot get exe dir")   // apps/
        .parent().expect("cannot get project root") // project root
        .to_path_buf()
}

fn find_bridge_script() -> PathBuf {
    // exe is at apps/Tau.exe
    // bridge is at apps/pet/bridge.py
    std::env::current_exe()
        .expect("cannot get exe path")
        .parent().expect("cannot get exe dir")
        .join("pet").join("bridge.py")
}
...
fn settings_path() -> PathBuf {
    dirs::home_dir()
        ...
        .join(".tau_desktop_settings.json")
}
...
fn start_bridge_with_config(app_handle: tauri::AppHandle, python_path: String, project_dir: String) -> Result<(), String> {
    ...
    let script = dir.join("apps").join("pet").join("bridge.py");
    if !script.exists() {
        return Err(format!("bridge.py not found at {:?}", script));
    }
