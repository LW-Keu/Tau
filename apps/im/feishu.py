...
APP_ID = str(taukeys.get("fs_app_id", "") or "").strip()
APP_SECRET = str(taukeys.get("fs_app_secret", "") or "").strip()
ALLOWED_USERS = _to_allowed_set(taukeys.get("fs_allowed_users", []))
PUBLIC_ACCESS = not ALLOWED_USERS or "*" in ALLOWED_USERS
AGENT_TIMEOUT_SEC = 900

agent = Tau()
_agent_thread = threading.Thread(target=agent.run, daemon=True, name="Tau-core")
_agent_thread.start()
try:
    print(f"[INFO] Tau core started pid={os.getpid()} thread_alive={_agent_thread.is_alive()} llm={agent.get_llm_name()}")
except Exception as e:
    print(f"[WARN] Tau core started but status unavailable: {e}")
...
def main():
    ...
    if not APP_ID or not APP_SECRET:
        print("错误: 请在 .tau/taukey.py 中配置 fs_app_id 和 fs_app_secret（可运行 `tau configure` 配置）")
        sys.exit(1)
