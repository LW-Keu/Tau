...
def _wrapper(): return _WRAPPER_EN if os.environ.get('TAU_LANG', os.environ.get('GA_LANG', '')) == 'en' else _WRAPPER_ZH
...
def handle_frontend_command(agent, query) -> str:
    """Sync entry for apps wanting a string back (tg/wx/stapp/...)."""
    question = _strip_cmd(query)
    if not question or question in ('help', '?', '-h', '--help'):
        return _help_text()
    started = time.time()
