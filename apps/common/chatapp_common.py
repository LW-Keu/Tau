...
def require_runtime(agent, label, **required):
    ...
    if missing:
        print(f"[{label}] ERROR: please set {', '.join(missing)} in .tau/taukey.py — run `tau configure`")
        sys.exit(1)
    if agent.llmclient is None:
        print(f"[{label}] ERROR: no usable LLM backend found in .tau/taukey.py — run `tau configure`")
        sys.exit(1)
...
_install_continue(_Tau)
from .btw_cmd import handle_frontend_command as _handle_btw_frontend, install as _install_btw; _install_btw(_Tau)
from .review_cmd import install as _install_review; _install_review(_Tau)
