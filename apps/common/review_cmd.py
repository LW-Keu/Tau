...
prompt 与 SOP 仅来自 `memories/review_sop/`,作为独立公共入口,不读取其他工作流的私有 prompt。
"""
...
_STUB_FALLBACK = (
    ...
    '# 本轮用户请求\n{user_request}\n\n'
    '请按 memories/code_review_principles.md 评审,直接 echo 报告到对话。\n'
    '不要写 review.md,不要打 [ROUND END]。'
)

def _render_prompt(user_request: str) -> str:
    """加载 /review inline prompt 并注入 user_request + tau_root。"""
    lang = os.environ.get('TAU_LANG', '').strip().lower()
    fname = _INLINE_PROMPT_EN if lang == 'en' else _INLINE_PROMPT_ZH
    fpath = os.path.join(CODE_ROOT, 'memories', _PROMPT_DIR, fname)
    tau_root = CODE_ROOT.replace('\\', '/')
    try:
        with open(fpath, 'r', encoding='utf-8') as f:
            return f.read().format(user_request=user_request, tau_root=tau_root)
...
def _help_text() -> str:
    return (
        ...
        '  `/review 审 apps 目录下所有改过的文件`\n\n'
        '产出:直接对话 markdown(不写文件、不开 subagent)。\n'
        '协议: `memories/review_sop/review_inline_prompt.txt` + `memories/code_review_principles.md`'
    )
