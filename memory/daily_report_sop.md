...
### 2.2b B路兜底/Stub 常踩点 (v3.2 实战验证, 2026-08-25)
- **E.4-01**: stub body 的 `url` 字段**必须**以 `http://` 开头（validator 拒 `https://`）。推荐占位 `http://example.com`
- **E.4-08**: body 段首必须含 `日期 + (报道|表示|声明|发布) + :`。`权威信源报道`、`来源通报`、`据报道` 等替代词不被识别；段首句式建议直接抄 §2.2 模板
- **E.4-09**: body 全文禁用 2-6 个连续大写字母（`SOS`/`WHO`/`USA`/`NATO`/`ASEAN` 等缩写会被命中）。命中即 FAIL
- **E.4-15**: body 严格 181-220 字（闭区间），L=180 / L=221 仍 FAIL。建议写 195-205
- **E.4-16**: `trends` 必须是 dict, 三段每段 200-300 总 600-900；任一段为空或超界即 FAIL
- **render 路径白名单**: cwd 必须在 `~/Tau` 才能用相对路径 `temp/output/<YYYYMMDD>/`；用绝对路径直接报 `output_dir 不在白名单下`
