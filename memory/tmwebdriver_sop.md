...
## Site Skills（域名技能库）
`TMWebDriver` 内置技能系统，把验证过的JS片段按域名持久化，下次直接复用。

```python
d = TMWebDriver()
d.set_session('bilibili')

# 执行并保存（首次）
d.execute_and_save('search',
    "window.location.href='https://search.bilibili.com/all?keyword={{keyword}}'",
    description='B站搜索', keyword='AI工具')

# 后续复用
d.execute_skill('search', keyword='Python教程')

# 查看所有技能
d.list_skills()                     # 所有域名
d.list_skills('bilibili.com')       # 指定域名

# 手动保存/读取
d.save_skill('get_title', 'document.title', description='获取页面标题')
...
pub.publish_video(
    ...
    tags=['AI'], platforms=['bilibili', 'douyin'],
    auto_publish=False   # True则自动点发布按钮
)
...
- 依赖 TMWebDriver 的 `/file` 路由向浏览器提供本地文件（HTTP :18766）
- `auto_publish=False` 时仅填写内容，不点发布，方便人工确认
- 上传进度靠 `_wait_for_text('上传完成')` 轮询，大文件可能耗时较长
...
## tab卡死/工具无响应: 一键软重启(R36实测)
- 现象：web_execute_js返回 "No response data in 15s" 但WS连接健康
- 根因：content script在目标tab卡死/未注入；扩展WS链路本身是好的
- ⭐**最快恢复**（无需重启Chrome/重装扩展）：
  ```js
  web_execute_js script='{"cmd":"cdp","tabId":N,"method":"Page.reload","params":{"ignoreCache":true}}'
  ```
  等待~3s让content script重新注入，之后工具恢复正常
