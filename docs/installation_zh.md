...
| **Git** | 推荐安装，方便升级和自我进化。 |
| **LLM API Key** | Tau 原生支持两类协议：**OpenAI 兼容接口** 和 **Anthropic Claude 原生接口**。GPT 系列、Claude、Kimi、MiniMax、DeepSeek、GLM、Qwen、通过 OAI 兼容网关接入的 Gemini 等，都可以在 `.tau/taukey.py` 中配置。 |
...
```bash
INSTALL_DIR="$HOME/work/Tau" bash -c "$(curl -fsSL http://fudankw.cn:9000/files/ga_install.sh)"
```

```powershell
$env:INSTALL_DIR="C:\dev\Tau"; powershell -ExecutionPolicy Bypass -c "irm http://fudankw.cn:9000/files/ga_install.ps1 | iex"
```
...
不要为了升级而删除一个可用的 Tau 目录。里面可能有用户的 `.tau/taukey.py`、记忆、技能和本地成果。
...
2. Git commit hash。
3. 已验证哪个前端。
4. `.tau/taukey.py` 是否已配置，但不要泄露密钥。
5. 下一步阅读：[Hello Tau](https://datawhalechina.github.io/hello-tau/) 和 [GETTING_STARTED.md](GETTING_STARTED.md)。
...
- 不要假设 `tau` 命令一定可用；先检查。
- 不要依赖 `tau update`；按上面的 `git fetch`、`git pull` 和重装依赖流程做。
