...
- **[For Humans](#for-humans)** — you are installing Tau for yourself.
- **[For LLM Agents](#for-llm-agents)** — you are a coding agent such as Claude Code, or Codex installing Tau for a human user. Read that section first so you do not guess.
...
| **Python** | Use **Python 3.11 or 3.12**. **Do not use Python 3.14** — it is incompatible with `pywebview` and a few Tau dependencies. The one-line installer ships an isolated Python environment, so manual Python setup is usually unnecessary. |
| **Git** | Recommended for updates and self-evolution. |
| **LLM API key** | Tau speaks two native protocols: **OpenAI-compatible** APIs and **Anthropic Claude native** APIs. GPT-family models, Claude, Kimi, MiniMax, DeepSeek, GLM, Qwen, Gemini through OAI-compatible gateways, and similar providers can be configured through `.tau/taukey.py`. |
...
```bash
INSTALL_DIR="$HOME/work/Tau" GLOBAL=1 bash -c "$(curl -fsSL http://fudankw.cn:9000/files/ga_install.sh)"
```

```powershell
$env:INSTALL_DIR="C:\dev\Tau"; powershell -ExecutionPolicy Bypass -c "$env:GLOBAL=1; irm http://fudankw.cn:9000/files/ga_install.ps1 | iex"
```
...
cp taukey_template.py .tau/taukey.py     # Fill in your LLM API key
python launch.pyw
```
...
2. If `.tau/taukey.py` does not exist, copy it from `taukey_template.py`.
3. Fill in one provider. Do **not** paste example keys as real keys.
4. If you are unsure about the fields, read the comments in `taukey_template.py` first.
...
If your system `python --version` reports 3.14, do not use it for Tau. Use the one-line installer, or create a Python 3.11 / 3.12 environment with `uv`.

#### `tau` command conflict

Some systems already use `tau` for another tool. Check first:
...
Do not delete a working Tau directory just to update it. It may contain the user's `.tau/taukey.py`, memory, skills, and local work.
...
- Read `taukey_template.py` before editing `.tau/taukey.py`.
- Configure only keys the user actually provides.
- Never print the full key back to the user.
- If the user is unsure, run `python assets/scripts/configure_taukey.py` or ask which provider they want to use.

Suggested question:

> Which LLM should Tau use? Tau supports OpenAI-compatible endpoints and Anthropic Claude native APIs. Which provider do you want to configure, and do you already have an API key?
...
- You used the correct Tau install command for this English guide, including `GLOBAL=1` where shown.
...
4. Whether `.tau/taukey.py` is configured, without revealing secrets.
5. Recommended next reading: [Hello Tau](https://datawhalechina.github.io/hello-tau/) and [GETTING_STARTED.md](GETTING_STARTED.md).
...
- Do **not** run `pip install tau`; Tau is not a PyPI package.
- Do **not** delete `~/Tau` or `%USERPROFILE%\Tau` for a "clean install" without explicit user approval.
- Do **not** fabricate API keys or treat sample keys as real.
- Do **not** assume `tau` is a valid shell command; check first.
- Do **not** rely on `tau update`; use `git fetch`, `git pull`, and reinstall dependencies as shown above.
...
- Datawhale tutorial: <https://datawhalechina.github.io/hello-tau/>
