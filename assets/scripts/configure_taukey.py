...
LLM_PROVIDERS = [
    ...
    },
    {
        'id': 'minimax',
        'name': 'MiniMax (双协议)',
        'desc': 'MiniMax，支持 Anthropic 和 OpenAI 双协议',
        'type': 'native_claude',
        'template': {
        ...
        'model_choices': ['MiniMax-M3', 'MiniMax-M2.7', 'MiniMax-M2.5'],
        'extra_fields': [
            {
                'key': '_protocol', 'label': '选择 API 协议',
                'type': 'choice',
                'options': [
                    {'id': 'native_claude', 'name': 'Anthropic 协议 (推荐)', 'desc': '原生 Claude 兼容；M3 在此端点实际 200K context（需 1M 选 OpenAI 协议）', 'apibase': 'https://api.minimaxi.com/anthropic'},
...
PLATFORMS = [
    {
        ...
        'desc': '直接用 python taumain.py 在终端交互',
        'deps': [],
    },
    {
        'id': 'telegram',
...
def _parse_existing_taukey():
    ...
    解析失败时返回 ([], [])
    """
    if not os.path.exists(TAUKPY_PATH):
        return [], []

    with open(TAUKPY_PATH, 'r', encoding='utf-8') as f:
        content = f.read()
...
def _backup_with_name(model_names, platform_ids):
    ...
    shutil.copy2(TAUKPY_PATH, backup_path)
    return backup_path
...
def main():
    ...
    # 备份旧文件
    if os.path.exists(TAUKPY_PATH):
        backup = os.path.join(TAU_DIR, f'taukey.py.bak.{datetime.now().strftime("%Y%m%d_%H%M%S")}')
        shutil.copy2(TAUKPY_PATH, backup)
        print(f"\n  {C['green']}✓ 旧配置已备份至:{C['reset']} {C['dim']}{backup}{C['reset']}")

    # 写入
    with open(TAUKPY_PATH, 'w', encoding='utf-8') as f:
        f.write(content)
    os.chmod(TAUKPY_PATH, 0o600)
    print(f"\n  {C['green']}✓ .tau/taukey.py 已生成!{C['reset']}")
    ...
    print()
    if llm_cfgs:
        print(f"  {C['cyan']}  终端 REPL:{C['reset']}  python taumain.py")
    if platform_configs:
